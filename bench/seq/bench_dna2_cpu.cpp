    ...
    try {
        ...
        if (iterations < 1 || motif_length < 1 || motif_length > 32) {
            throw std::runtime_error(
                "usage: baseplaneDna2CpuBench [count] [iterations] [seed] [threads, 0=hardware] "
                "[motif_length 1..32] [max_mismatches]");
        }

        const baseplane::bench::benchmark_mutex_guard guard("baseplane-dna2-cpu");
        ...
        std::printf("seed=%u\n", seed);
        std::printf("backend=%s\n", BASEPLANE_ENABLE_HIGHWAY ? "highway" : "scalar");
