...
#if defined(__CUDACC__)
#define BASEPLANE_SEQ_HD __host__ __device__ __forceinline__
#define BASEPLANE_SEQ_DEVICE __device__ __forceinline__
#else
#define BASEPLANE_SEQ_HD inline
#endif
...
BASEPLANE_SEQ_HD int popcount64(std::uint64_t value) {
#if defined(__CUDA_ARCH__)
...
BASEPLANE_SEQ_HD std::uint64_t spread_active_mask_to_packed_fields(std::uint32_t active_mask) {
...
BASEPLANE_SEQ_HD int word64_mismatches_packed_count_fields(
    dna2_word64 a,
    dna2_word64 b,
...
BASEPLANE_SEQ_HD std::uint8_t make_base(char c) {
...
BASEPLANE_SEQ_HD char base_to_char(std::uint8_t b) {
...
BASEPLANE_SEQ_HD int planes32_mismatches(dna2_planes32 a, dna2_planes32 b, std::uint32_t active_mask) {
...
BASEPLANE_SEQ_HD int word64_mismatches(dna2_word64 a, dna2_word64 b, std::uint32_t active_mask_32bases) {
    return detail::popcount32(word64_mismatch_mask(a, b, active_mask_32bases));
}

BASEPLANE_SEQ_HD int word64_mismatches_packed_count(dna2_word64 a, dna2_word64 b, std::uint32_t active_mask_32bases) {
